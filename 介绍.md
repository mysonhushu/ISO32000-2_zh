...
3.16
不推荐使用的内容
ISO 32000的一部分，不应写入PDF 2.0文档，而应由PDF处理器忽略
注1：在某些情况下，本文档中明确说明了这些对继续使用不赞成使用的功能的限制的变化。
注释2：实施人员请注意，在更高版本的ISO 32000或子集标准（例如PDF / X（ISO）中，本文档中不建议使用的某些功能可能对其施加更严格的约束，甚至完全删除。15930），PDF / A（ISO 19005），PDF / E（ISO 24517），
...
[资料来源：资料来源：ISO / IEC 9541-1：2012，3.12]]
3.31
图形状态
图形控制参数的下推堆栈的顶部，该参数定义了图形操作员将在其中执行的当前全局框架
3.32
ICC个人资料
引用ISO 15076-1：2010或任何ICC.1规范的手段
3.33
ICC规范
跨平台配置文件格式，用于根据ISO 15076-1：2010或ICC.1规范创建和解释颜色数据
3.34
间接宾语
...
3.45
宾语
可从中构造PDF文件的基本数据结构，包括以下类型：数组，布尔值，字典，整数，名称，null，实数，流和字符串
3.46
对象参考
...
3.57
长方形
用于描述页面和边框上各种对象的位置的特定数组对象，并写为四个数字的数组，给出一对对角相对的角的坐标，通常以[ llx lly urx ury ] 的形式指定较低的矩形的-左x，左下y，右上x和右上y坐标，按此顺序
3.58
资源字典
...
3.67
空格字符
分隔PDF语法结构（例如名称和数字）的字符；空白字符是水平制表符（09h），行进纸（0Ah），格式进纸（0Ch），回车（0Dh），空格（20h），空（00h）; （请参见7.2.3 “字符集”中的“ 表1-空白字符”）
3.68
XFDF
ISO 19444-1：2016的参考方法，文档管理-XML 表单数据格式-第1部分：ISO 32000-2（XFDF 3.0）的使用
3.69
XMP
引用ISO 16684-1的方法，Adobe XMP：可扩展元数据平台，第2部分：标准架构，以及Adobe XMP：可扩展元数据平台，第3部分：文件存储
3.70
XMP封包
...
[10]	ISO / IEC 9541-1：2012，信息技术—字体信息交换—第1部分：体系结构
[11]	ISO 14289-1：2014，文档管理应用程序-电子文档文件格式增强，以实现可访问性-第1部分：ISO 32000-1的使用（PDF / UA-1）
[12]	ISO 19005-1 / Cor 1 / Cor 2，文档管理-长期保存的电子文档文件格式-第1部分：PDF 1.4的使用（PDF / A-1）
[13]	19005-2，文档管理-长期保存的电子文档文件格式-第2部分：ISO 32000-1的使用（PDF / A-2）
[14]	ISO 19005-3，文件管理-长期保存的电子文件格式-第3部分：使用 ISO 32000-1（PDF / A-3）
[15]	ISO 16612-2，图形技术—可变数据交换—第2部分：使用PDF / X-4和PDF / X-5（PDF / VT-1和PDF / VT-2）
[16]	ISO 24517-1：2008，文档管理-使用PDF的工程文档格式-第1部分：PDF 1.6的使用（PDF / E-1）
...
1了PostScript®是Adobe Systems Incorporated的商标。提供此信息是为了方便本文档的用户，并不构成ISO对所命名产品的认可。
1这些文档可以在PDF协会的https://reference.pdfa.org/iso/32000或Adobe Systems Incorporated网站http://partners.adobe.com/public/developer/pdf/上找到。 index_bibliography.html。
1这些文档可以在PDF协会的https://reference.pdfa.org/iso/32000或Adobe Systems Incorporated网站http://partners.adobe.com/public/developer/pdf/上找到。 index_bibliography.html。
2这些FIPS文档可从NIST的http://www.nist.gov/itl/fipscurrent.cfm获得。
3这些文档可作为Internet工程任务组RFC在https://www.ietf.org/获得。
4这些文档可从http://www.etsi.org获得。
